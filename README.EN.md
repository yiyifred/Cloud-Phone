...
- **Android app**: on first server setup, defaults to the LAN gateway (`.1` on the current subnet, e.g. `192.168.31.1`) on port `3000`; after connect, routes to first-time password setup or login like the web UI; saves the password encrypted on device for auto sign-in next launch; console UI uses bottom tabs for Devices and Settings
