...
- **Android 客户端**：首次填写服务器时，默认地址为本机网段网关（末段为 `.1`，如 `192.168.31.1`），默认端口 `3000`；连接成功后自动区分首次改密（仍为 `admin`）与登录；登录成功后密码加密保存在本机，下次自动验证登录；进入控制台后底部 Tab 为「设备」「设置」；设备页布局与 Web 移动版画廊一致（横向卡片、列表/截图自动刷新）；支持「+」添加设备（USB / 配对码 / 二维码）
