...
当前版本：**v0.12.12** · Node 后端 + Vue 3 前端 · 基于 [scrcpy](https://github.com/Genymobile/scrcpy) 4.0 自编译 WebSocket 投屏
...
- **Android 客户端**：首次填写服务器时，默认地址为本机网段网关（末段为 `.1`，如 `192.168.31.1`），默认端口 `3000`；连接成功后自动区分首次改密（仍为 `admin`）与登录；登录成功后密码加密保存在本机，下次自动验证登录；进入控制台后底部 Tab 为「设备」「设置」；设备页布局与 Web 移动版画廊一致（横向卡片、列表/截图自动刷新）；支持「+」添加设备（USB / 配对码 / 二维码）；界面图标统一使用 Community Material（与 Web MDI 一致）；点击设备进入详情页，可配置镜像/摄像头投屏参数（多标签设置，与 Web 工作区左侧一致，含虚拟屏预设与 `start_app`）；点击「开始」进入全屏实时投屏（H.264 解码、触控注入、镜像导航/摄像头手电变焦工具栏、Material 动效与自动隐藏顶栏，流参数与 Web 对齐）
- **后端本地数据**：`backend/node/data/` 存放加密密钥与密码库，仅本机使用，勿提交到 Git
